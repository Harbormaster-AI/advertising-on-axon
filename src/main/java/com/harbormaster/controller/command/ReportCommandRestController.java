/*
 * Generated by Harbormaster
 *
 *
 * Blueprint:
 * Axon4-Framework-Server
 *
 * Domain Model:
 * Advertising Industry Domain Model
 *
 * Do not edit manually.
 ********************************************************
 *  Confidential
 *
 * 2026 
 * All Rights Reserved.
 *
 * This file is subject to the terms and conditions defined in
 * file 'license.txt', which is part of this source code package.
 *
 * Contributors :
 *        - General Release
 */
package com.harbormaster.controller.command;

import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.web.bind.annotation.*;

import java.util.*;
import java.util.concurrent.*;
import java.util.logging.Level;
import java.util.logging.Logger;

import com.harbormaster.api.*;
import com.harbormaster.service.*;
import com.harbormaster.entity.*;
import com.harbormaster.exception.*;
import com.harbormaster.projector.*;

import com.harbormaster.controller.*;

/**
 * <h2>ReportCommandRestController as REST API</h2>
 * <p>
 * Implements Spring Controller command CQRS processing for entity Report.
 * </p>
  * Generated By Harbormaster
 *
 * Generated From
 *
 *      <h3>Blueprint</h3>
 * 			<table>
 *          <tr><td>name</td><td>Axon4-Framework-Server</td></tr>
 *          <tr><td>published</td><td>09/08/2026</td></tr>
 *          <tr><td>design pattern</td><td>CQRS</td></tr>
 *          <tr><td>architecture style</td><td>EventDrivenArchitecture</td></tr>
 *          </table>
 *
 *      <h3>Domain Model</h3>
 *          <table>
 *          <tr><td>name</td><td>Advertising Industry Domain Model</td></tr>
 *          <tr><td>description</td><td>HCIDL model for advertising planning, buying, trafficking, delivery, and measurement.</td></tr>
 *          <tr><td>industry</td><td>Advertising</td></tr>
 *          <tr><td>version</td><td>1.0.0</td></tr>
 *          <table>
 *
 * <h3>Services Used</h3>
 *
 *  	ReportService
 *
 * <h3>Produces</h3>
 *
 *      <p>application/json</p>
 *
 * <h3>Commands Accepted</h3>
 *      <ul>
 *      <li>CreateReportCommand</li>
 *      <li>UpdateReportCommand</li>
 *      <liDeleteReportCommand</li>
 *      </ul>
 *
 * <p>@author Harbormaster Dev Team</p>
 */

@CrossOrigin
@RestController
@RequestMapping("/ReportCommand")
public class ReportCommandRestController extends BaseSpringRestController {

	public ReportCommandRestController( ReportService service ) {
		this.service = service;
	}
	
    /**
     * Handles create a Report.
     * @param		Report	report
     * @return		CompletableFuture<UUID> 
     */
	@PostMapping("/create")
    public CompletableFuture<UUID> create( @RequestBody(required=true) CreateReportCommand command ) {
		CompletableFuture<UUID> completableFuture = null;
		try {

			completableFuture = service.createReport( command );
        }
        catch( Throwable exc ) {
        	LOGGER.log( Level.WARNING, exc.getMessage(), exc );        	
        }
		
		return completableFuture;
    }

    /**
     * Handles updating a Report.  if no key provided, calls create, otherwise calls save
     * @param		Report report
     * @return		CompletableFuture<Void>
     */
	@PutMapping("/update")
    public CompletableFuture<Void> update( @RequestBody(required=true) UpdateReportCommand command ) {
		CompletableFuture<Void> completableFuture = null;
		try {                        	        
			// -----------------------------------------------
			// delegate the UpdateReportCommand
			// -----------------------------------------------
			completableFuture = service.updateReport(command);;
	    }
	    catch( Throwable exc ) {
	    	LOGGER.log( Level.WARNING, "ReportController:update() - successfully update Report - " + exc.getMessage());        	
	    }		
		
		return completableFuture;
	}
 
    /**
     * Handles deleting a Report entity
     * @param		command ${class.getDeleteCommandAlias()}
     * @return		CompletableFuture<Void>
     */
    @DeleteMapping("/delete")    
    public CompletableFuture<Void> delete( @RequestParam(required=true) UUID reportId  ) {
    	CompletableFuture<Void> completableFuture = null;
		DeleteReportCommand command = new DeleteReportCommand( reportId );

    	try {
        	ReportService delegate = service;

        	completableFuture = delegate.delete( command );
    		LOGGER.log( Level.WARNING, "Successfully deleted Report with key " + command.getReportId() );
        }
        catch( Throwable exc ) {
        	LOGGER.log( Level.WARNING, exc.getMessage() );
        }
        
        return completableFuture;
	}        
	

    /**
     * save AdAccount on Report
     * @param		command AssignAdAccountToReportCommand
     */     
	@PutMapping("/assignAdAccount")
	public void assignAdAccount( @RequestBody AssignAdAccountToReportCommand command ) {
		try {
			service.assignAdAccount( command );   
		}
        catch( Throwable exc ) {
        	LOGGER.log( Level.WARNING, "Failed to assign AdAccount", exc );
        }
	}

    /**
     * unassign AdAccount on Report
     * @param		 command UnAssignAdAccountFromReportCommand
     */     
	@PutMapping("/unAssignAdAccount")
	public void unAssignAdAccount( @RequestBody(required=true)  UnAssignAdAccountFromReportCommand command ) {
		try {
			service.unAssignAdAccount( command );   
		}
		catch( Exception exc ) {
			LOGGER.log( Level.WARNING, "Failed to unassign AdAccount", exc );
		}
	}
	
    /**
     * save Campaign on Report
     * @param		command AssignCampaignToReportCommand
     */     
	@PutMapping("/assignCampaign")
	public void assignCampaign( @RequestBody AssignCampaignToReportCommand command ) {
		try {
			service.assignCampaign( command );   
		}
        catch( Throwable exc ) {
        	LOGGER.log( Level.WARNING, "Failed to assign Campaign", exc );
        }
	}

    /**
     * unassign Campaign on Report
     * @param		 command UnAssignCampaignFromReportCommand
     */     
	@PutMapping("/unAssignCampaign")
	public void unAssignCampaign( @RequestBody(required=true)  UnAssignCampaignFromReportCommand command ) {
		try {
			service.unAssignCampaign( command );   
		}
		catch( Exception exc ) {
			LOGGER.log( Level.WARNING, "Failed to unassign Campaign", exc );
		}
	}
	
    /**
     * save LineItem on Report
     * @param		command AssignLineItemToReportCommand
     */     
	@PutMapping("/assignLineItem")
	public void assignLineItem( @RequestBody AssignLineItemToReportCommand command ) {
		try {
			service.assignLineItem( command );   
		}
        catch( Throwable exc ) {
        	LOGGER.log( Level.WARNING, "Failed to assign LineItem", exc );
        }
	}

    /**
     * unassign LineItem on Report
     * @param		 command UnAssignLineItemFromReportCommand
     */     
	@PutMapping("/unAssignLineItem")
	public void unAssignLineItem( @RequestBody(required=true)  UnAssignLineItemFromReportCommand command ) {
		try {
			service.unAssignLineItem( command );   
		}
		catch( Exception exc ) {
			LOGGER.log( Level.WARNING, "Failed to unassign LineItem", exc );
		}
	}
	


//************************************************************************    
// Attributes
//************************************************************************
    protected Report report = null;
	protected ReportService service = null;
    private static final Logger LOGGER = Logger.getLogger(ReportCommandRestController.class.getName());
    
}
