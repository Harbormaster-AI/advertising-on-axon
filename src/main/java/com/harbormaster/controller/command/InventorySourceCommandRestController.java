/*
 * Generated by Harbormaster
 *
 *
 * Blueprint:
 * Axon4-Framework-Server
 *
 * Domain Model:
 * Advertising Industry Domain Model
 *
 * Do not edit manually.
 ********************************************************
 *  Confidential
 *
 * 2026 
 * All Rights Reserved.
 *
 * This file is subject to the terms and conditions defined in
 * file 'license.txt', which is part of this source code package.
 *
 * Contributors :
 *        - General Release
 */
package com.harbormaster.controller.command;

import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.web.bind.annotation.*;

import java.util.*;
import java.util.concurrent.*;
import java.util.logging.Level;
import java.util.logging.Logger;

import com.harbormaster.api.*;
import com.harbormaster.service.*;
import com.harbormaster.entity.*;
import com.harbormaster.exception.*;
import com.harbormaster.projector.*;

import com.harbormaster.controller.*;

/**
 * <h2>InventorySourceCommandRestController as REST API</h2>
 * <p>
 * Implements Spring Controller command CQRS processing for entity InventorySource.
 * </p>
  * Generated By Harbormaster
 *
 * Generated From
 *
 *      <h3>Blueprint</h3>
 * 			<table>
 *          <tr><td>name</td><td>Axon4-Framework-Server</td></tr>
 *          <tr><td>published</td><td>09/08/2026</td></tr>
 *          <tr><td>design pattern</td><td>CQRS</td></tr>
 *          <tr><td>architecture style</td><td>EventDrivenArchitecture</td></tr>
 *          </table>
 *
 *      <h3>Domain Model</h3>
 *          <table>
 *          <tr><td>name</td><td>Advertising Industry Domain Model</td></tr>
 *          <tr><td>description</td><td>HCIDL model for advertising planning, buying, trafficking, delivery, and measurement.</td></tr>
 *          <tr><td>industry</td><td>Advertising</td></tr>
 *          <tr><td>version</td><td>1.0.0</td></tr>
 *          <table>
 *
 * <h3>Services Used</h3>
 *
 *  	InventorySourceService
 *
 * <h3>Produces</h3>
 *
 *      <p>application/json</p>
 *
 * <h3>Commands Accepted</h3>
 *      <ul>
 *      <li>CreateInventorySourceCommand</li>
 *      <li>UpdateInventorySourceCommand</li>
 *      <liDeleteInventorySourceCommand</li>
 *      </ul>
 *
 * <p>@author Harbormaster Dev Team</p>
 */

@CrossOrigin
@RestController
@RequestMapping("/InventorySourceCommand")
public class InventorySourceCommandRestController extends BaseSpringRestController {

	public InventorySourceCommandRestController( InventorySourceService service ) {
		this.service = service;
	}
	
    /**
     * Handles create a InventorySource.
     * @param		InventorySource	inventorySource
     * @return		CompletableFuture<UUID> 
     */
	@PostMapping("/create")
    public CompletableFuture<UUID> create( @RequestBody(required=true) CreateInventorySourceCommand command ) {
		CompletableFuture<UUID> completableFuture = null;
		try {

			completableFuture = service.createInventorySource( command );
        }
        catch( Throwable exc ) {
        	LOGGER.log( Level.WARNING, exc.getMessage(), exc );        	
        }
		
		return completableFuture;
    }

    /**
     * Handles updating a InventorySource.  if no key provided, calls create, otherwise calls save
     * @param		InventorySource inventorySource
     * @return		CompletableFuture<Void>
     */
	@PutMapping("/update")
    public CompletableFuture<Void> update( @RequestBody(required=true) UpdateInventorySourceCommand command ) {
		CompletableFuture<Void> completableFuture = null;
		try {                        	        
			// -----------------------------------------------
			// delegate the UpdateInventorySourceCommand
			// -----------------------------------------------
			completableFuture = service.updateInventorySource(command);;
	    }
	    catch( Throwable exc ) {
	    	LOGGER.log( Level.WARNING, "InventorySourceController:update() - successfully update InventorySource - " + exc.getMessage());        	
	    }		
		
		return completableFuture;
	}
 
    /**
     * Handles deleting a InventorySource entity
     * @param		command ${class.getDeleteCommandAlias()}
     * @return		CompletableFuture<Void>
     */
    @DeleteMapping("/delete")    
    public CompletableFuture<Void> delete( @RequestParam(required=true) UUID inventorySourceId  ) {
    	CompletableFuture<Void> completableFuture = null;
		DeleteInventorySourceCommand command = new DeleteInventorySourceCommand( inventorySourceId );

    	try {
        	InventorySourceService delegate = service;

        	completableFuture = delegate.delete( command );
    		LOGGER.log( Level.WARNING, "Successfully deleted InventorySource with key " + command.getInventorySourceId() );
        }
        catch( Throwable exc ) {
        	LOGGER.log( Level.WARNING, exc.getMessage() );
        }
        
        return completableFuture;
	}        
	

    /**
     * save Publisher on InventorySource
     * @param		command AssignPublisherToInventorySourceCommand
     */     
	@PutMapping("/assignPublisher")
	public void assignPublisher( @RequestBody AssignPublisherToInventorySourceCommand command ) {
		try {
			service.assignPublisher( command );   
		}
        catch( Throwable exc ) {
        	LOGGER.log( Level.WARNING, "Failed to assign Publisher", exc );
        }
	}

    /**
     * unassign Publisher on InventorySource
     * @param		 command UnAssignPublisherFromInventorySourceCommand
     */     
	@PutMapping("/unAssignPublisher")
	public void unAssignPublisher( @RequestBody(required=true)  UnAssignPublisherFromInventorySourceCommand command ) {
		try {
			service.unAssignPublisher( command );   
		}
		catch( Exception exc ) {
			LOGGER.log( Level.WARNING, "Failed to unassign Publisher", exc );
		}
	}
	

    /**
     * save AdSlots on InventorySource
     * @param		command AssignAdSlotsToInventorySourceCommand
     */     
	@PutMapping("/addToAdSlots")
	public void addToAdSlots( @RequestBody(required=true) AssignAdSlotsToInventorySourceCommand command ) {
		try {
			service.addToAdSlots( command );   
		}
		catch( Exception exc ) {
			LOGGER.log( Level.WARNING, "Failed to add to Set AdSlots", exc );
		}
	}

    /**
     * remove AdSlots on InventorySource
     * @param		command RemoveAdSlotsFromInventorySourceCommand
     */     	
	@PutMapping("/removeFromAdSlots")
	public void removeFromAdSlots( 	@RequestBody(required=true) RemoveAdSlotsFromInventorySourceCommand command )
	{		
		try {
			service.removeFromAdSlots( command );
		}
		catch( Exception exc ) {
			LOGGER.log( Level.WARNING, "Failed to remove from Set AdSlots", exc );
		}
	}

    /**
     * save Deals on InventorySource
     * @param		command AssignDealsToInventorySourceCommand
     */     
	@PutMapping("/addToDeals")
	public void addToDeals( @RequestBody(required=true) AssignDealsToInventorySourceCommand command ) {
		try {
			service.addToDeals( command );   
		}
		catch( Exception exc ) {
			LOGGER.log( Level.WARNING, "Failed to add to Set Deals", exc );
		}
	}

    /**
     * remove Deals on InventorySource
     * @param		command RemoveDealsFromInventorySourceCommand
     */     	
	@PutMapping("/removeFromDeals")
	public void removeFromDeals( 	@RequestBody(required=true) RemoveDealsFromInventorySourceCommand command )
	{		
		try {
			service.removeFromDeals( command );
		}
		catch( Exception exc ) {
			LOGGER.log( Level.WARNING, "Failed to remove from Set Deals", exc );
		}
	}


//************************************************************************    
// Attributes
//************************************************************************
    protected InventorySource inventorySource = null;
	protected InventorySourceService service = null;
    private static final Logger LOGGER = Logger.getLogger(InventorySourceCommandRestController.class.getName());
    
}
