/*
 * Generated by Harbormaster
 *
 *
 * Blueprint:
 * Axon4-Framework-Server
 *
 * Domain Model:
 * Advertising Industry Domain Model
 *
 * Do not edit manually.
 ********************************************************
 *  Confidential
 *
 * 2026 
 * All Rights Reserved.
 *
 * This file is subject to the terms and conditions defined in
 * file 'license.txt', which is part of this source code package.
 *
 * Contributors :
 *        - General Release
 */
package com.harbormaster.controller.command;

import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.web.bind.annotation.*;

import java.util.*;
import java.util.concurrent.*;
import java.util.logging.Level;
import java.util.logging.Logger;

import com.harbormaster.api.*;
import com.harbormaster.service.*;
import com.harbormaster.entity.*;
import com.harbormaster.exception.*;
import com.harbormaster.projector.*;

import com.harbormaster.controller.*;

/**
 * <h2>InsertionOrderCommandRestController as REST API</h2>
 * <p>
 * Implements Spring Controller command CQRS processing for entity InsertionOrder.
 * </p>
  * Generated By Harbormaster
 *
 * Generated From
 *
 *      <h3>Blueprint</h3>
 * 			<table>
 *          <tr><td>name</td><td>Axon4-Framework-Server</td></tr>
 *          <tr><td>published</td><td>09/08/2026</td></tr>
 *          <tr><td>design pattern</td><td>CQRS</td></tr>
 *          <tr><td>architecture style</td><td>EventDrivenArchitecture</td></tr>
 *          </table>
 *
 *      <h3>Domain Model</h3>
 *          <table>
 *          <tr><td>name</td><td>Advertising Industry Domain Model</td></tr>
 *          <tr><td>description</td><td>HCIDL model for advertising planning, buying, trafficking, delivery, and measurement.</td></tr>
 *          <tr><td>industry</td><td>Advertising</td></tr>
 *          <tr><td>version</td><td>1.0.0</td></tr>
 *          <table>
 *
 * <h3>Services Used</h3>
 *
 *  	InsertionOrderService
 *
 * <h3>Produces</h3>
 *
 *      <p>application/json</p>
 *
 * <h3>Commands Accepted</h3>
 *      <ul>
 *      <li>CreateInsertionOrderCommand</li>
 *      <li>UpdateInsertionOrderCommand</li>
 *      <liDeleteInsertionOrderCommand</li>
 *      </ul>
 *
 * <p>@author Harbormaster Dev Team</p>
 */

@CrossOrigin
@RestController
@RequestMapping("/InsertionOrderCommand")
public class InsertionOrderCommandRestController extends BaseSpringRestController {

	public InsertionOrderCommandRestController( InsertionOrderService service ) {
		this.service = service;
	}
	
    /**
     * Handles create a InsertionOrder.
     * @param		InsertionOrder	insertionOrder
     * @return		CompletableFuture<UUID> 
     */
	@PostMapping("/create")
    public CompletableFuture<UUID> create( @RequestBody(required=true) CreateInsertionOrderCommand command ) {
		CompletableFuture<UUID> completableFuture = null;
		try {

			completableFuture = service.createInsertionOrder( command );
        }
        catch( Throwable exc ) {
        	LOGGER.log( Level.WARNING, exc.getMessage(), exc );        	
        }
		
		return completableFuture;
    }

    /**
     * Handles updating a InsertionOrder.  if no key provided, calls create, otherwise calls save
     * @param		InsertionOrder insertionOrder
     * @return		CompletableFuture<Void>
     */
	@PutMapping("/update")
    public CompletableFuture<Void> update( @RequestBody(required=true) UpdateInsertionOrderCommand command ) {
		CompletableFuture<Void> completableFuture = null;
		try {                        	        
			// -----------------------------------------------
			// delegate the UpdateInsertionOrderCommand
			// -----------------------------------------------
			completableFuture = service.updateInsertionOrder(command);;
	    }
	    catch( Throwable exc ) {
	    	LOGGER.log( Level.WARNING, "InsertionOrderController:update() - successfully update InsertionOrder - " + exc.getMessage());        	
	    }		
		
		return completableFuture;
	}
 
    /**
     * Handles deleting a InsertionOrder entity
     * @param		command ${class.getDeleteCommandAlias()}
     * @return		CompletableFuture<Void>
     */
    @DeleteMapping("/delete")    
    public CompletableFuture<Void> delete( @RequestParam(required=true) UUID insertionOrderId  ) {
    	CompletableFuture<Void> completableFuture = null;
		DeleteInsertionOrderCommand command = new DeleteInsertionOrderCommand( insertionOrderId );

    	try {
        	InsertionOrderService delegate = service;

        	completableFuture = delegate.delete( command );
    		LOGGER.log( Level.WARNING, "Successfully deleted InsertionOrder with key " + command.getInsertionOrderId() );
        }
        catch( Throwable exc ) {
        	LOGGER.log( Level.WARNING, exc.getMessage() );
        }
        
        return completableFuture;
	}        
	

    /**
     * save Advertiser on InsertionOrder
     * @param		command AssignAdvertiserToInsertionOrderCommand
     */     
	@PutMapping("/assignAdvertiser")
	public void assignAdvertiser( @RequestBody AssignAdvertiserToInsertionOrderCommand command ) {
		try {
			service.assignAdvertiser( command );   
		}
        catch( Throwable exc ) {
        	LOGGER.log( Level.WARNING, "Failed to assign Advertiser", exc );
        }
	}

    /**
     * unassign Advertiser on InsertionOrder
     * @param		 command UnAssignAdvertiserFromInsertionOrderCommand
     */     
	@PutMapping("/unAssignAdvertiser")
	public void unAssignAdvertiser( @RequestBody(required=true)  UnAssignAdvertiserFromInsertionOrderCommand command ) {
		try {
			service.unAssignAdvertiser( command );   
		}
		catch( Exception exc ) {
			LOGGER.log( Level.WARNING, "Failed to unassign Advertiser", exc );
		}
	}
	
    /**
     * save Agency on InsertionOrder
     * @param		command AssignAgencyToInsertionOrderCommand
     */     
	@PutMapping("/assignAgency")
	public void assignAgency( @RequestBody AssignAgencyToInsertionOrderCommand command ) {
		try {
			service.assignAgency( command );   
		}
        catch( Throwable exc ) {
        	LOGGER.log( Level.WARNING, "Failed to assign Agency", exc );
        }
	}

    /**
     * unassign Agency on InsertionOrder
     * @param		 command UnAssignAgencyFromInsertionOrderCommand
     */     
	@PutMapping("/unAssignAgency")
	public void unAssignAgency( @RequestBody(required=true)  UnAssignAgencyFromInsertionOrderCommand command ) {
		try {
			service.unAssignAgency( command );   
		}
		catch( Exception exc ) {
			LOGGER.log( Level.WARNING, "Failed to unassign Agency", exc );
		}
	}
	
    /**
     * save Publisher on InsertionOrder
     * @param		command AssignPublisherToInsertionOrderCommand
     */     
	@PutMapping("/assignPublisher")
	public void assignPublisher( @RequestBody AssignPublisherToInsertionOrderCommand command ) {
		try {
			service.assignPublisher( command );   
		}
        catch( Throwable exc ) {
        	LOGGER.log( Level.WARNING, "Failed to assign Publisher", exc );
        }
	}

    /**
     * unassign Publisher on InsertionOrder
     * @param		 command UnAssignPublisherFromInsertionOrderCommand
     */     
	@PutMapping("/unAssignPublisher")
	public void unAssignPublisher( @RequestBody(required=true)  UnAssignPublisherFromInsertionOrderCommand command ) {
		try {
			service.unAssignPublisher( command );   
		}
		catch( Exception exc ) {
			LOGGER.log( Level.WARNING, "Failed to unassign Publisher", exc );
		}
	}
	

    /**
     * save Campaigns on InsertionOrder
     * @param		command AssignCampaignsToInsertionOrderCommand
     */     
	@PutMapping("/addToCampaigns")
	public void addToCampaigns( @RequestBody(required=true) AssignCampaignsToInsertionOrderCommand command ) {
		try {
			service.addToCampaigns( command );   
		}
		catch( Exception exc ) {
			LOGGER.log( Level.WARNING, "Failed to add to Set Campaigns", exc );
		}
	}

    /**
     * remove Campaigns on InsertionOrder
     * @param		command RemoveCampaignsFromInsertionOrderCommand
     */     	
	@PutMapping("/removeFromCampaigns")
	public void removeFromCampaigns( 	@RequestBody(required=true) RemoveCampaignsFromInsertionOrderCommand command )
	{		
		try {
			service.removeFromCampaigns( command );
		}
		catch( Exception exc ) {
			LOGGER.log( Level.WARNING, "Failed to remove from Set Campaigns", exc );
		}
	}


//************************************************************************    
// Attributes
//************************************************************************
    protected InsertionOrder insertionOrder = null;
	protected InsertionOrderService service = null;
    private static final Logger LOGGER = Logger.getLogger(InsertionOrderCommandRestController.class.getName());
    
}
