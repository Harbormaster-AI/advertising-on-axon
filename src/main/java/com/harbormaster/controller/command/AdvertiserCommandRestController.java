/*
 * Generated by Harbormaster
 *
 *
 * Blueprint:
 * Axon4-Framework-Server
 *
 * Domain Model:
 * Advertising Industry Domain Model
 *
 * Do not edit manually.
 ********************************************************
 *  Confidential
 *
 * 2026 
 * All Rights Reserved.
 *
 * This file is subject to the terms and conditions defined in
 * file 'license.txt', which is part of this source code package.
 *
 * Contributors :
 *        - General Release
 */
package com.harbormaster.controller.command;

import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.web.bind.annotation.*;

import java.util.*;
import java.util.concurrent.*;
import java.util.logging.Level;
import java.util.logging.Logger;

import com.harbormaster.api.*;
import com.harbormaster.service.*;
import com.harbormaster.entity.*;
import com.harbormaster.exception.*;
import com.harbormaster.projector.*;

import com.harbormaster.controller.*;

/**
 * <h2>AdvertiserCommandRestController as REST API</h2>
 * <p>
 * Implements Spring Controller command CQRS processing for entity Advertiser.
 * </p>
  * Generated By Harbormaster
 *
 * Generated From
 *
 *      <h3>Blueprint</h3>
 * 			<table>
 *          <tr><td>name</td><td>Axon4-Framework-Server</td></tr>
 *          <tr><td>published</td><td>09/08/2026</td></tr>
 *          <tr><td>design pattern</td><td>CQRS</td></tr>
 *          <tr><td>architecture style</td><td>EventDrivenArchitecture</td></tr>
 *          </table>
 *
 *      <h3>Domain Model</h3>
 *          <table>
 *          <tr><td>name</td><td>Advertising Industry Domain Model</td></tr>
 *          <tr><td>description</td><td>HCIDL model for advertising planning, buying, trafficking, delivery, and measurement.</td></tr>
 *          <tr><td>industry</td><td>Advertising</td></tr>
 *          <tr><td>version</td><td>1.0.0</td></tr>
 *          <table>
 *
 * <h3>Services Used</h3>
 *
 *  	AdvertiserService
 *
 * <h3>Produces</h3>
 *
 *      <p>application/json</p>
 *
 * <h3>Commands Accepted</h3>
 *      <ul>
 *      <li>CreateAdvertiserCommand</li>
 *      <li>UpdateAdvertiserCommand</li>
 *      <liDeleteAdvertiserCommand</li>
 *      </ul>
 *
 * <p>@author Harbormaster Dev Team</p>
 */

@CrossOrigin
@RestController
@RequestMapping("/AdvertiserCommand")
public class AdvertiserCommandRestController extends BaseSpringRestController {

	public AdvertiserCommandRestController( AdvertiserService service ) {
		this.service = service;
	}
	
    /**
     * Handles create a Advertiser.
     * @param		Advertiser	advertiser
     * @return		CompletableFuture<UUID> 
     */
	@PostMapping("/create")
    public CompletableFuture<UUID> create( @RequestBody(required=true) CreateAdvertiserCommand command ) {
		CompletableFuture<UUID> completableFuture = null;
		try {

			completableFuture = service.createAdvertiser( command );
        }
        catch( Throwable exc ) {
        	LOGGER.log( Level.WARNING, exc.getMessage(), exc );        	
        }
		
		return completableFuture;
    }

    /**
     * Handles updating a Advertiser.  if no key provided, calls create, otherwise calls save
     * @param		Advertiser advertiser
     * @return		CompletableFuture<Void>
     */
	@PutMapping("/update")
    public CompletableFuture<Void> update( @RequestBody(required=true) UpdateAdvertiserCommand command ) {
		CompletableFuture<Void> completableFuture = null;
		try {                        	        
			// -----------------------------------------------
			// delegate the UpdateAdvertiserCommand
			// -----------------------------------------------
			completableFuture = service.updateAdvertiser(command);;
	    }
	    catch( Throwable exc ) {
	    	LOGGER.log( Level.WARNING, "AdvertiserController:update() - successfully update Advertiser - " + exc.getMessage());        	
	    }		
		
		return completableFuture;
	}
 
    /**
     * Handles deleting a Advertiser entity
     * @param		command ${class.getDeleteCommandAlias()}
     * @return		CompletableFuture<Void>
     */
    @DeleteMapping("/delete")    
    public CompletableFuture<Void> delete( @RequestParam(required=true) UUID advertiserId  ) {
    	CompletableFuture<Void> completableFuture = null;
		DeleteAdvertiserCommand command = new DeleteAdvertiserCommand( advertiserId );

    	try {
        	AdvertiserService delegate = service;

        	completableFuture = delegate.delete( command );
    		LOGGER.log( Level.WARNING, "Successfully deleted Advertiser with key " + command.getAdvertiserId() );
        }
        catch( Throwable exc ) {
        	LOGGER.log( Level.WARNING, exc.getMessage() );
        }
        
        return completableFuture;
	}        
	

    /**
     * save Agency on Advertiser
     * @param		command AssignAgencyToAdvertiserCommand
     */     
	@PutMapping("/assignAgency")
	public void assignAgency( @RequestBody AssignAgencyToAdvertiserCommand command ) {
		try {
			service.assignAgency( command );   
		}
        catch( Throwable exc ) {
        	LOGGER.log( Level.WARNING, "Failed to assign Agency", exc );
        }
	}

    /**
     * unassign Agency on Advertiser
     * @param		 command UnAssignAgencyFromAdvertiserCommand
     */     
	@PutMapping("/unAssignAgency")
	public void unAssignAgency( @RequestBody(required=true)  UnAssignAgencyFromAdvertiserCommand command ) {
		try {
			service.unAssignAgency( command );   
		}
		catch( Exception exc ) {
			LOGGER.log( Level.WARNING, "Failed to unassign Agency", exc );
		}
	}
	

    /**
     * save AdAccounts on Advertiser
     * @param		command AssignAdAccountsToAdvertiserCommand
     */     
	@PutMapping("/addToAdAccounts")
	public void addToAdAccounts( @RequestBody(required=true) AssignAdAccountsToAdvertiserCommand command ) {
		try {
			service.addToAdAccounts( command );   
		}
		catch( Exception exc ) {
			LOGGER.log( Level.WARNING, "Failed to add to Set AdAccounts", exc );
		}
	}

    /**
     * remove AdAccounts on Advertiser
     * @param		command RemoveAdAccountsFromAdvertiserCommand
     */     	
	@PutMapping("/removeFromAdAccounts")
	public void removeFromAdAccounts( 	@RequestBody(required=true) RemoveAdAccountsFromAdvertiserCommand command )
	{		
		try {
			service.removeFromAdAccounts( command );
		}
		catch( Exception exc ) {
			LOGGER.log( Level.WARNING, "Failed to remove from Set AdAccounts", exc );
		}
	}

    /**
     * save BillingProfiles on Advertiser
     * @param		command AssignBillingProfilesToAdvertiserCommand
     */     
	@PutMapping("/addToBillingProfiles")
	public void addToBillingProfiles( @RequestBody(required=true) AssignBillingProfilesToAdvertiserCommand command ) {
		try {
			service.addToBillingProfiles( command );   
		}
		catch( Exception exc ) {
			LOGGER.log( Level.WARNING, "Failed to add to Set BillingProfiles", exc );
		}
	}

    /**
     * remove BillingProfiles on Advertiser
     * @param		command RemoveBillingProfilesFromAdvertiserCommand
     */     	
	@PutMapping("/removeFromBillingProfiles")
	public void removeFromBillingProfiles( 	@RequestBody(required=true) RemoveBillingProfilesFromAdvertiserCommand command )
	{		
		try {
			service.removeFromBillingProfiles( command );
		}
		catch( Exception exc ) {
			LOGGER.log( Level.WARNING, "Failed to remove from Set BillingProfiles", exc );
		}
	}

    /**
     * save Campaigns on Advertiser
     * @param		command AssignCampaignsToAdvertiserCommand
     */     
	@PutMapping("/addToCampaigns")
	public void addToCampaigns( @RequestBody(required=true) AssignCampaignsToAdvertiserCommand command ) {
		try {
			service.addToCampaigns( command );   
		}
		catch( Exception exc ) {
			LOGGER.log( Level.WARNING, "Failed to add to Set Campaigns", exc );
		}
	}

    /**
     * remove Campaigns on Advertiser
     * @param		command RemoveCampaignsFromAdvertiserCommand
     */     	
	@PutMapping("/removeFromCampaigns")
	public void removeFromCampaigns( 	@RequestBody(required=true) RemoveCampaignsFromAdvertiserCommand command )
	{		
		try {
			service.removeFromCampaigns( command );
		}
		catch( Exception exc ) {
			LOGGER.log( Level.WARNING, "Failed to remove from Set Campaigns", exc );
		}
	}

    /**
     * save TrackingPixels on Advertiser
     * @param		command AssignTrackingPixelsToAdvertiserCommand
     */     
	@PutMapping("/addToTrackingPixels")
	public void addToTrackingPixels( @RequestBody(required=true) AssignTrackingPixelsToAdvertiserCommand command ) {
		try {
			service.addToTrackingPixels( command );   
		}
		catch( Exception exc ) {
			LOGGER.log( Level.WARNING, "Failed to add to Set TrackingPixels", exc );
		}
	}

    /**
     * remove TrackingPixels on Advertiser
     * @param		command RemoveTrackingPixelsFromAdvertiserCommand
     */     	
	@PutMapping("/removeFromTrackingPixels")
	public void removeFromTrackingPixels( 	@RequestBody(required=true) RemoveTrackingPixelsFromAdvertiserCommand command )
	{		
		try {
			service.removeFromTrackingPixels( command );
		}
		catch( Exception exc ) {
			LOGGER.log( Level.WARNING, "Failed to remove from Set TrackingPixels", exc );
		}
	}


//************************************************************************    
// Attributes
//************************************************************************
    protected Advertiser advertiser = null;
	protected AdvertiserService service = null;
    private static final Logger LOGGER = Logger.getLogger(AdvertiserCommandRestController.class.getName());
    
}
