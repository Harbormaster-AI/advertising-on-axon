/*
 * Generated by Harbormaster
 *
 *
 * Blueprint:
 * Axon4-Framework-Server
 *
 * Domain Model:
 * Advertising Industry Domain Model
 *
 * Do not edit manually.
 ********************************************************
 *  Confidential
 *
 * 2026 
 * All Rights Reserved.
 *
 * This file is subject to the terms and conditions defined in
 * file 'license.txt', which is part of this source code package.
 *
 * Contributors :
 *        - General Release
 */
package com.harbormaster.controller.command;

import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.web.bind.annotation.*;

import java.util.*;
import java.util.concurrent.*;
import java.util.logging.Level;
import java.util.logging.Logger;

import com.harbormaster.api.*;
import com.harbormaster.service.*;
import com.harbormaster.entity.*;
import com.harbormaster.exception.*;
import com.harbormaster.projector.*;

import com.harbormaster.controller.*;

/**
 * <h2>BrandSafetyPolicyCommandRestController as REST API</h2>
 * <p>
 * Implements Spring Controller command CQRS processing for entity BrandSafetyPolicy.
 * </p>
  * Generated By Harbormaster
 *
 * Generated From
 *
 *      <h3>Blueprint</h3>
 * 			<table>
 *          <tr><td>name</td><td>Axon4-Framework-Server</td></tr>
 *          <tr><td>published</td><td>09/08/2026</td></tr>
 *          <tr><td>design pattern</td><td>CQRS</td></tr>
 *          <tr><td>architecture style</td><td>EventDrivenArchitecture</td></tr>
 *          </table>
 *
 *      <h3>Domain Model</h3>
 *          <table>
 *          <tr><td>name</td><td>Advertising Industry Domain Model</td></tr>
 *          <tr><td>description</td><td>HCIDL model for advertising planning, buying, trafficking, delivery, and measurement.</td></tr>
 *          <tr><td>industry</td><td>Advertising</td></tr>
 *          <tr><td>version</td><td>1.0.0</td></tr>
 *          <table>
 *
 * <h3>Services Used</h3>
 *
 *  	BrandSafetyPolicyService
 *
 * <h3>Produces</h3>
 *
 *      <p>application/json</p>
 *
 * <h3>Commands Accepted</h3>
 *      <ul>
 *      <li>CreateBrandSafetyPolicyCommand</li>
 *      <li>UpdateBrandSafetyPolicyCommand</li>
 *      <liDeleteBrandSafetyPolicyCommand</li>
 *      </ul>
 *
 * <p>@author Harbormaster Dev Team</p>
 */

@CrossOrigin
@RestController
@RequestMapping("/BrandSafetyPolicyCommand")
public class BrandSafetyPolicyCommandRestController extends BaseSpringRestController {

	public BrandSafetyPolicyCommandRestController( BrandSafetyPolicyService service ) {
		this.service = service;
	}
	
    /**
     * Handles create a BrandSafetyPolicy.
     * @param		BrandSafetyPolicy	brandSafetyPolicy
     * @return		CompletableFuture<UUID> 
     */
	@PostMapping("/create")
    public CompletableFuture<UUID> create( @RequestBody(required=true) CreateBrandSafetyPolicyCommand command ) {
		CompletableFuture<UUID> completableFuture = null;
		try {

			completableFuture = service.createBrandSafetyPolicy( command );
        }
        catch( Throwable exc ) {
        	LOGGER.log( Level.WARNING, exc.getMessage(), exc );        	
        }
		
		return completableFuture;
    }

    /**
     * Handles updating a BrandSafetyPolicy.  if no key provided, calls create, otherwise calls save
     * @param		BrandSafetyPolicy brandSafetyPolicy
     * @return		CompletableFuture<Void>
     */
	@PutMapping("/update")
    public CompletableFuture<Void> update( @RequestBody(required=true) UpdateBrandSafetyPolicyCommand command ) {
		CompletableFuture<Void> completableFuture = null;
		try {                        	        
			// -----------------------------------------------
			// delegate the UpdateBrandSafetyPolicyCommand
			// -----------------------------------------------
			completableFuture = service.updateBrandSafetyPolicy(command);;
	    }
	    catch( Throwable exc ) {
	    	LOGGER.log( Level.WARNING, "BrandSafetyPolicyController:update() - successfully update BrandSafetyPolicy - " + exc.getMessage());        	
	    }		
		
		return completableFuture;
	}
 
    /**
     * Handles deleting a BrandSafetyPolicy entity
     * @param		command ${class.getDeleteCommandAlias()}
     * @return		CompletableFuture<Void>
     */
    @DeleteMapping("/delete")    
    public CompletableFuture<Void> delete( @RequestParam(required=true) UUID brandSafetyPolicyId  ) {
    	CompletableFuture<Void> completableFuture = null;
		DeleteBrandSafetyPolicyCommand command = new DeleteBrandSafetyPolicyCommand( brandSafetyPolicyId );

    	try {
        	BrandSafetyPolicyService delegate = service;

        	completableFuture = delegate.delete( command );
    		LOGGER.log( Level.WARNING, "Successfully deleted BrandSafetyPolicy with key " + command.getBrandSafetyPolicyId() );
        }
        catch( Throwable exc ) {
        	LOGGER.log( Level.WARNING, exc.getMessage() );
        }
        
        return completableFuture;
	}        
	


    /**
     * save TargetingProfiles on BrandSafetyPolicy
     * @param		command AssignTargetingProfilesToBrandSafetyPolicyCommand
     */     
	@PutMapping("/addToTargetingProfiles")
	public void addToTargetingProfiles( @RequestBody(required=true) AssignTargetingProfilesToBrandSafetyPolicyCommand command ) {
		try {
			service.addToTargetingProfiles( command );   
		}
		catch( Exception exc ) {
			LOGGER.log( Level.WARNING, "Failed to add to Set TargetingProfiles", exc );
		}
	}

    /**
     * remove TargetingProfiles on BrandSafetyPolicy
     * @param		command RemoveTargetingProfilesFromBrandSafetyPolicyCommand
     */     	
	@PutMapping("/removeFromTargetingProfiles")
	public void removeFromTargetingProfiles( 	@RequestBody(required=true) RemoveTargetingProfilesFromBrandSafetyPolicyCommand command )
	{		
		try {
			service.removeFromTargetingProfiles( command );
		}
		catch( Exception exc ) {
			LOGGER.log( Level.WARNING, "Failed to remove from Set TargetingProfiles", exc );
		}
	}


//************************************************************************    
// Attributes
//************************************************************************
    protected BrandSafetyPolicy brandSafetyPolicy = null;
	protected BrandSafetyPolicyService service = null;
    private static final Logger LOGGER = Logger.getLogger(BrandSafetyPolicyCommandRestController.class.getName());
    
}
