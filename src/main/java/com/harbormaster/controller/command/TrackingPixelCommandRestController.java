/*
 * Generated by Harbormaster
 *
 *
 * Blueprint:
 * Axon4-Framework-Server
 *
 * Domain Model:
 * Advertising Industry Domain Model
 *
 * Do not edit manually.
 ********************************************************
 *  Confidential
 *
 * 2026 
 * All Rights Reserved.
 *
 * This file is subject to the terms and conditions defined in
 * file 'license.txt', which is part of this source code package.
 *
 * Contributors :
 *        - General Release
 */
package com.harbormaster.controller.command;

import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.web.bind.annotation.*;

import java.util.*;
import java.util.concurrent.*;
import java.util.logging.Level;
import java.util.logging.Logger;

import com.harbormaster.api.*;
import com.harbormaster.service.*;
import com.harbormaster.entity.*;
import com.harbormaster.exception.*;
import com.harbormaster.projector.*;

import com.harbormaster.controller.*;

/**
 * <h2>TrackingPixelCommandRestController as REST API</h2>
 * <p>
 * Implements Spring Controller command CQRS processing for entity TrackingPixel.
 * </p>
  * Generated By Harbormaster
 *
 * Generated From
 *
 *      <h3>Blueprint</h3>
 * 			<table>
 *          <tr><td>name</td><td>Axon4-Framework-Server</td></tr>
 *          <tr><td>published</td><td>09/08/2026</td></tr>
 *          <tr><td>design pattern</td><td>CQRS</td></tr>
 *          <tr><td>architecture style</td><td>EventDrivenArchitecture</td></tr>
 *          </table>
 *
 *      <h3>Domain Model</h3>
 *          <table>
 *          <tr><td>name</td><td>Advertising Industry Domain Model</td></tr>
 *          <tr><td>description</td><td>HCIDL model for advertising planning, buying, trafficking, delivery, and measurement.</td></tr>
 *          <tr><td>industry</td><td>Advertising</td></tr>
 *          <tr><td>version</td><td>1.0.0</td></tr>
 *          <table>
 *
 * <h3>Services Used</h3>
 *
 *  	TrackingPixelService
 *
 * <h3>Produces</h3>
 *
 *      <p>application/json</p>
 *
 * <h3>Commands Accepted</h3>
 *      <ul>
 *      <li>CreateTrackingPixelCommand</li>
 *      <li>UpdateTrackingPixelCommand</li>
 *      <liDeleteTrackingPixelCommand</li>
 *      </ul>
 *
 * <p>@author Harbormaster Dev Team</p>
 */

@CrossOrigin
@RestController
@RequestMapping("/TrackingPixelCommand")
public class TrackingPixelCommandRestController extends BaseSpringRestController {

	public TrackingPixelCommandRestController( TrackingPixelService service ) {
		this.service = service;
	}
	
    /**
     * Handles create a TrackingPixel.
     * @param		TrackingPixel	trackingPixel
     * @return		CompletableFuture<UUID> 
     */
	@PostMapping("/create")
    public CompletableFuture<UUID> create( @RequestBody(required=true) CreateTrackingPixelCommand command ) {
		CompletableFuture<UUID> completableFuture = null;
		try {

			completableFuture = service.createTrackingPixel( command );
        }
        catch( Throwable exc ) {
        	LOGGER.log( Level.WARNING, exc.getMessage(), exc );        	
        }
		
		return completableFuture;
    }

    /**
     * Handles updating a TrackingPixel.  if no key provided, calls create, otherwise calls save
     * @param		TrackingPixel trackingPixel
     * @return		CompletableFuture<Void>
     */
	@PutMapping("/update")
    public CompletableFuture<Void> update( @RequestBody(required=true) UpdateTrackingPixelCommand command ) {
		CompletableFuture<Void> completableFuture = null;
		try {                        	        
			// -----------------------------------------------
			// delegate the UpdateTrackingPixelCommand
			// -----------------------------------------------
			completableFuture = service.updateTrackingPixel(command);;
	    }
	    catch( Throwable exc ) {
	    	LOGGER.log( Level.WARNING, "TrackingPixelController:update() - successfully update TrackingPixel - " + exc.getMessage());        	
	    }		
		
		return completableFuture;
	}
 
    /**
     * Handles deleting a TrackingPixel entity
     * @param		command ${class.getDeleteCommandAlias()}
     * @return		CompletableFuture<Void>
     */
    @DeleteMapping("/delete")    
    public CompletableFuture<Void> delete( @RequestParam(required=true) UUID trackingPixelId  ) {
    	CompletableFuture<Void> completableFuture = null;
		DeleteTrackingPixelCommand command = new DeleteTrackingPixelCommand( trackingPixelId );

    	try {
        	TrackingPixelService delegate = service;

        	completableFuture = delegate.delete( command );
    		LOGGER.log( Level.WARNING, "Successfully deleted TrackingPixel with key " + command.getTrackingPixelId() );
        }
        catch( Throwable exc ) {
        	LOGGER.log( Level.WARNING, exc.getMessage() );
        }
        
        return completableFuture;
	}        
	

    /**
     * save Campaign on TrackingPixel
     * @param		command AssignCampaignToTrackingPixelCommand
     */     
	@PutMapping("/assignCampaign")
	public void assignCampaign( @RequestBody AssignCampaignToTrackingPixelCommand command ) {
		try {
			service.assignCampaign( command );   
		}
        catch( Throwable exc ) {
        	LOGGER.log( Level.WARNING, "Failed to assign Campaign", exc );
        }
	}

    /**
     * unassign Campaign on TrackingPixel
     * @param		 command UnAssignCampaignFromTrackingPixelCommand
     */     
	@PutMapping("/unAssignCampaign")
	public void unAssignCampaign( @RequestBody(required=true)  UnAssignCampaignFromTrackingPixelCommand command ) {
		try {
			service.unAssignCampaign( command );   
		}
		catch( Exception exc ) {
			LOGGER.log( Level.WARNING, "Failed to unassign Campaign", exc );
		}
	}
	
    /**
     * save Advertiser on TrackingPixel
     * @param		command AssignAdvertiserToTrackingPixelCommand
     */     
	@PutMapping("/assignAdvertiser")
	public void assignAdvertiser( @RequestBody AssignAdvertiserToTrackingPixelCommand command ) {
		try {
			service.assignAdvertiser( command );   
		}
        catch( Throwable exc ) {
        	LOGGER.log( Level.WARNING, "Failed to assign Advertiser", exc );
        }
	}

    /**
     * unassign Advertiser on TrackingPixel
     * @param		 command UnAssignAdvertiserFromTrackingPixelCommand
     */     
	@PutMapping("/unAssignAdvertiser")
	public void unAssignAdvertiser( @RequestBody(required=true)  UnAssignAdvertiserFromTrackingPixelCommand command ) {
		try {
			service.unAssignAdvertiser( command );   
		}
		catch( Exception exc ) {
			LOGGER.log( Level.WARNING, "Failed to unassign Advertiser", exc );
		}
	}
	

    /**
     * save ConversionEvents on TrackingPixel
     * @param		command AssignConversionEventsToTrackingPixelCommand
     */     
	@PutMapping("/addToConversionEvents")
	public void addToConversionEvents( @RequestBody(required=true) AssignConversionEventsToTrackingPixelCommand command ) {
		try {
			service.addToConversionEvents( command );   
		}
		catch( Exception exc ) {
			LOGGER.log( Level.WARNING, "Failed to add to Set ConversionEvents", exc );
		}
	}

    /**
     * remove ConversionEvents on TrackingPixel
     * @param		command RemoveConversionEventsFromTrackingPixelCommand
     */     	
	@PutMapping("/removeFromConversionEvents")
	public void removeFromConversionEvents( 	@RequestBody(required=true) RemoveConversionEventsFromTrackingPixelCommand command )
	{		
		try {
			service.removeFromConversionEvents( command );
		}
		catch( Exception exc ) {
			LOGGER.log( Level.WARNING, "Failed to remove from Set ConversionEvents", exc );
		}
	}


//************************************************************************    
// Attributes
//************************************************************************
    protected TrackingPixel trackingPixel = null;
	protected TrackingPixelService service = null;
    private static final Logger LOGGER = Logger.getLogger(TrackingPixelCommandRestController.class.getName());
    
}
