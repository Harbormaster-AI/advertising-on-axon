/*
 * Generated by Harbormaster
 *
 *
 * Blueprint:
 * Axon4-Framework-Server
 *
 * Domain Model:
 * Advertising Industry Domain Model
 *
 * Do not edit manually.
 ********************************************************
 *  Confidential
 *
 * 2026 
 * All Rights Reserved.
 *
 * This file is subject to the terms and conditions defined in
 * file 'license.txt', which is part of this source code package.
 *
 * Contributors :
 *        - General Release
 */
package com.harbormaster.controller.command;

import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.web.bind.annotation.*;

import java.util.*;
import java.util.concurrent.*;
import java.util.logging.Level;
import java.util.logging.Logger;

import com.harbormaster.api.*;
import com.harbormaster.service.*;
import com.harbormaster.entity.*;
import com.harbormaster.exception.*;
import com.harbormaster.projector.*;

import com.harbormaster.controller.*;

/**
 * <h2>DealCommandRestController as REST API</h2>
 * <p>
 * Implements Spring Controller command CQRS processing for entity Deal.
 * </p>
  * Generated By Harbormaster
 *
 * Generated From
 *
 *      <h3>Blueprint</h3>
 * 			<table>
 *          <tr><td>name</td><td>Axon4-Framework-Server</td></tr>
 *          <tr><td>published</td><td>09/08/2026</td></tr>
 *          <tr><td>design pattern</td><td>CQRS</td></tr>
 *          <tr><td>architecture style</td><td>EventDrivenArchitecture</td></tr>
 *          </table>
 *
 *      <h3>Domain Model</h3>
 *          <table>
 *          <tr><td>name</td><td>Advertising Industry Domain Model</td></tr>
 *          <tr><td>description</td><td>HCIDL model for advertising planning, buying, trafficking, delivery, and measurement.</td></tr>
 *          <tr><td>industry</td><td>Advertising</td></tr>
 *          <tr><td>version</td><td>1.0.0</td></tr>
 *          <table>
 *
 * <h3>Services Used</h3>
 *
 *  	DealService
 *
 * <h3>Produces</h3>
 *
 *      <p>application/json</p>
 *
 * <h3>Commands Accepted</h3>
 *      <ul>
 *      <li>CreateDealCommand</li>
 *      <li>UpdateDealCommand</li>
 *      <liDeleteDealCommand</li>
 *      </ul>
 *
 * <p>@author Harbormaster Dev Team</p>
 */

@CrossOrigin
@RestController
@RequestMapping("/DealCommand")
public class DealCommandRestController extends BaseSpringRestController {

	public DealCommandRestController( DealService service ) {
		this.service = service;
	}
	
    /**
     * Handles create a Deal.
     * @param		Deal	deal
     * @return		CompletableFuture<UUID> 
     */
	@PostMapping("/create")
    public CompletableFuture<UUID> create( @RequestBody(required=true) CreateDealCommand command ) {
		CompletableFuture<UUID> completableFuture = null;
		try {

			completableFuture = service.createDeal( command );
        }
        catch( Throwable exc ) {
        	LOGGER.log( Level.WARNING, exc.getMessage(), exc );        	
        }
		
		return completableFuture;
    }

    /**
     * Handles updating a Deal.  if no key provided, calls create, otherwise calls save
     * @param		Deal deal
     * @return		CompletableFuture<Void>
     */
	@PutMapping("/update")
    public CompletableFuture<Void> update( @RequestBody(required=true) UpdateDealCommand command ) {
		CompletableFuture<Void> completableFuture = null;
		try {                        	        
			// -----------------------------------------------
			// delegate the UpdateDealCommand
			// -----------------------------------------------
			completableFuture = service.updateDeal(command);;
	    }
	    catch( Throwable exc ) {
	    	LOGGER.log( Level.WARNING, "DealController:update() - successfully update Deal - " + exc.getMessage());        	
	    }		
		
		return completableFuture;
	}
 
    /**
     * Handles deleting a Deal entity
     * @param		command ${class.getDeleteCommandAlias()}
     * @return		CompletableFuture<Void>
     */
    @DeleteMapping("/delete")    
    public CompletableFuture<Void> delete( @RequestParam(required=true) UUID dealId  ) {
    	CompletableFuture<Void> completableFuture = null;
		DeleteDealCommand command = new DeleteDealCommand( dealId );

    	try {
        	DealService delegate = service;

        	completableFuture = delegate.delete( command );
    		LOGGER.log( Level.WARNING, "Successfully deleted Deal with key " + command.getDealId() );
        }
        catch( Throwable exc ) {
        	LOGGER.log( Level.WARNING, exc.getMessage() );
        }
        
        return completableFuture;
	}        
	

    /**
     * save Publisher on Deal
     * @param		command AssignPublisherToDealCommand
     */     
	@PutMapping("/assignPublisher")
	public void assignPublisher( @RequestBody AssignPublisherToDealCommand command ) {
		try {
			service.assignPublisher( command );   
		}
        catch( Throwable exc ) {
        	LOGGER.log( Level.WARNING, "Failed to assign Publisher", exc );
        }
	}

    /**
     * unassign Publisher on Deal
     * @param		 command UnAssignPublisherFromDealCommand
     */     
	@PutMapping("/unAssignPublisher")
	public void unAssignPublisher( @RequestBody(required=true)  UnAssignPublisherFromDealCommand command ) {
		try {
			service.unAssignPublisher( command );   
		}
		catch( Exception exc ) {
			LOGGER.log( Level.WARNING, "Failed to unassign Publisher", exc );
		}
	}
	

    /**
     * save InventorySources on Deal
     * @param		command AssignInventorySourcesToDealCommand
     */     
	@PutMapping("/addToInventorySources")
	public void addToInventorySources( @RequestBody(required=true) AssignInventorySourcesToDealCommand command ) {
		try {
			service.addToInventorySources( command );   
		}
		catch( Exception exc ) {
			LOGGER.log( Level.WARNING, "Failed to add to Set InventorySources", exc );
		}
	}

    /**
     * remove InventorySources on Deal
     * @param		command RemoveInventorySourcesFromDealCommand
     */     	
	@PutMapping("/removeFromInventorySources")
	public void removeFromInventorySources( 	@RequestBody(required=true) RemoveInventorySourcesFromDealCommand command )
	{		
		try {
			service.removeFromInventorySources( command );
		}
		catch( Exception exc ) {
			LOGGER.log( Level.WARNING, "Failed to remove from Set InventorySources", exc );
		}
	}

    /**
     * save Placements on Deal
     * @param		command AssignPlacementsToDealCommand
     */     
	@PutMapping("/addToPlacements")
	public void addToPlacements( @RequestBody(required=true) AssignPlacementsToDealCommand command ) {
		try {
			service.addToPlacements( command );   
		}
		catch( Exception exc ) {
			LOGGER.log( Level.WARNING, "Failed to add to Set Placements", exc );
		}
	}

    /**
     * remove Placements on Deal
     * @param		command RemovePlacementsFromDealCommand
     */     	
	@PutMapping("/removeFromPlacements")
	public void removeFromPlacements( 	@RequestBody(required=true) RemovePlacementsFromDealCommand command )
	{		
		try {
			service.removeFromPlacements( command );
		}
		catch( Exception exc ) {
			LOGGER.log( Level.WARNING, "Failed to remove from Set Placements", exc );
		}
	}


//************************************************************************    
// Attributes
//************************************************************************
    protected Deal deal = null;
	protected DealService service = null;
    private static final Logger LOGGER = Logger.getLogger(DealCommandRestController.class.getName());
    
}
