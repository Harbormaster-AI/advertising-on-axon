/*
 * Generated by Harbormaster
 *
 *
 * Blueprint:
 * Axon4-Framework-Server
 *
 * Domain Model:
 * Advertising Industry Domain Model
 *
 * Do not edit manually.
 ********************************************************
 *  Confidential
 *
 * 2026 
 * All Rights Reserved.
 *
 * This file is subject to the terms and conditions defined in
 * file 'license.txt', which is part of this source code package.
 *
 * Contributors :
 *        - General Release
 */
package com.harbormaster.controller.command;

import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.web.bind.annotation.*;

import java.util.*;
import java.util.concurrent.*;
import java.util.logging.Level;
import java.util.logging.Logger;

import com.harbormaster.api.*;
import com.harbormaster.service.*;
import com.harbormaster.entity.*;
import com.harbormaster.exception.*;
import com.harbormaster.projector.*;

import com.harbormaster.controller.*;

/**
 * <h2>RateCommandRestController as REST API</h2>
 * <p>
 * Implements Spring Controller command CQRS processing for entity Rate.
 * </p>
  * Generated By Harbormaster
 *
 * Generated From
 *
 *      <h3>Blueprint</h3>
 * 			<table>
 *          <tr><td>name</td><td>Axon4-Framework-Server</td></tr>
 *          <tr><td>published</td><td>09/08/2026</td></tr>
 *          <tr><td>design pattern</td><td>CQRS</td></tr>
 *          <tr><td>architecture style</td><td>EventDrivenArchitecture</td></tr>
 *          </table>
 *
 *      <h3>Domain Model</h3>
 *          <table>
 *          <tr><td>name</td><td>Advertising Industry Domain Model</td></tr>
 *          <tr><td>description</td><td>HCIDL model for advertising planning, buying, trafficking, delivery, and measurement.</td></tr>
 *          <tr><td>industry</td><td>Advertising</td></tr>
 *          <tr><td>version</td><td>1.0.0</td></tr>
 *          <table>
 *
 * <h3>Services Used</h3>
 *
 *  	RateService
 *
 * <h3>Produces</h3>
 *
 *      <p>application/json</p>
 *
 * <h3>Commands Accepted</h3>
 *      <ul>
 *      <li>CreateRateCommand</li>
 *      <li>UpdateRateCommand</li>
 *      <liDeleteRateCommand</li>
 *      </ul>
 *
 * <p>@author Harbormaster Dev Team</p>
 */

@CrossOrigin
@RestController
@RequestMapping("/RateCommand")
public class RateCommandRestController extends BaseSpringRestController {

	public RateCommandRestController( RateService service ) {
		this.service = service;
	}
	
    /**
     * Handles create a Rate.
     * @param		Rate	rate
     * @return		CompletableFuture<UUID> 
     */
	@PostMapping("/create")
    public CompletableFuture<UUID> create( @RequestBody(required=true) CreateRateCommand command ) {
		CompletableFuture<UUID> completableFuture = null;
		try {

			completableFuture = service.createRate( command );
        }
        catch( Throwable exc ) {
        	LOGGER.log( Level.WARNING, exc.getMessage(), exc );        	
        }
		
		return completableFuture;
    }

    /**
     * Handles updating a Rate.  if no key provided, calls create, otherwise calls save
     * @param		Rate rate
     * @return		CompletableFuture<Void>
     */
	@PutMapping("/update")
    public CompletableFuture<Void> update( @RequestBody(required=true) UpdateRateCommand command ) {
		CompletableFuture<Void> completableFuture = null;
		try {                        	        
			// -----------------------------------------------
			// delegate the UpdateRateCommand
			// -----------------------------------------------
			completableFuture = service.updateRate(command);;
	    }
	    catch( Throwable exc ) {
	    	LOGGER.log( Level.WARNING, "RateController:update() - successfully update Rate - " + exc.getMessage());        	
	    }		
		
		return completableFuture;
	}
 
    /**
     * Handles deleting a Rate entity
     * @param		command ${class.getDeleteCommandAlias()}
     * @return		CompletableFuture<Void>
     */
    @DeleteMapping("/delete")    
    public CompletableFuture<Void> delete( @RequestParam(required=true) UUID rateId  ) {
    	CompletableFuture<Void> completableFuture = null;
		DeleteRateCommand command = new DeleteRateCommand( rateId );

    	try {
        	RateService delegate = service;

        	completableFuture = delegate.delete( command );
    		LOGGER.log( Level.WARNING, "Successfully deleted Rate with key " + command.getRateId() );
        }
        catch( Throwable exc ) {
        	LOGGER.log( Level.WARNING, exc.getMessage() );
        }
        
        return completableFuture;
	}        
	

    /**
     * save RateCard on Rate
     * @param		command AssignRateCardToRateCommand
     */     
	@PutMapping("/assignRateCard")
	public void assignRateCard( @RequestBody AssignRateCardToRateCommand command ) {
		try {
			service.assignRateCard( command );   
		}
        catch( Throwable exc ) {
        	LOGGER.log( Level.WARNING, "Failed to assign RateCard", exc );
        }
	}

    /**
     * unassign RateCard on Rate
     * @param		 command UnAssignRateCardFromRateCommand
     */     
	@PutMapping("/unAssignRateCard")
	public void unAssignRateCard( @RequestBody(required=true)  UnAssignRateCardFromRateCommand command ) {
		try {
			service.unAssignRateCard( command );   
		}
		catch( Exception exc ) {
			LOGGER.log( Level.WARNING, "Failed to unassign RateCard", exc );
		}
	}
	
    /**
     * save AdSlot on Rate
     * @param		command AssignAdSlotToRateCommand
     */     
	@PutMapping("/assignAdSlot")
	public void assignAdSlot( @RequestBody AssignAdSlotToRateCommand command ) {
		try {
			service.assignAdSlot( command );   
		}
        catch( Throwable exc ) {
        	LOGGER.log( Level.WARNING, "Failed to assign AdSlot", exc );
        }
	}

    /**
     * unassign AdSlot on Rate
     * @param		 command UnAssignAdSlotFromRateCommand
     */     
	@PutMapping("/unAssignAdSlot")
	public void unAssignAdSlot( @RequestBody(required=true)  UnAssignAdSlotFromRateCommand command ) {
		try {
			service.unAssignAdSlot( command );   
		}
		catch( Exception exc ) {
			LOGGER.log( Level.WARNING, "Failed to unassign AdSlot", exc );
		}
	}
	


//************************************************************************    
// Attributes
//************************************************************************
    protected Rate rate = null;
	protected RateService service = null;
    private static final Logger LOGGER = Logger.getLogger(RateCommandRestController.class.getName());
    
}
